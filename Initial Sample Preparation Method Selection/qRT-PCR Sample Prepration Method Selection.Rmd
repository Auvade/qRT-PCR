---
title: "qRT-PCR Sample Prepration Method Selection"
...
# setwd("path/to/your/data") # Uncomment and set your path if needed
...
regression_results <- data %>%
  ...
  summarize(
    lm_model = list(lm(Cq ~ LogTemplate, data = pick(everything()))), # Linear model
...
# Create plots for each Sample Prep and Gene
...
# Save plots for each Sample Prep and Gene
...
# Combine all plots into a single list to enable the grobs() function to handle them correctly
plots <- unlist(plots, recursive = FALSE)

# Arrange all plots in a grid layout
pdf("All_Plots.pdf", width = 11, height = 8.5) # Landscape layout
gridExtra::marrangeGrob(grobs = plots, nrow = 2, ncol = 3) %>% 
  print()
dev.off()

# Save all plots in a multi-page PDF
multi_page_plot <- marrangeGrob(grobs = plots, nrow = 2, ncol = 3, top = "qRT-PCR Sample Preparation Method Selection Plots")
